...
for i in range(num_episodes):
    ...
    while not done:
        ...
        if (epoch_steps > 0 and epoch_steps % batch_size == 0) or done:
            ...
            actor_loss, critic_loss, ent = train_actor_critic(tf.stack(states_memory[-trajectory_length:]),
    ...
    print(f'[epoch {i} ({epoch_steps})] Actor: {np.mean(actor_loss_history):.4f} Critic: {np.mean(critic_loss_history):.4f} Entropy: {np.mean(entropy_history):.4f} Total reward: {total_episod_reward} Mean(100)={last_mean:.4f}')
    if last_mean > 200:
        break
env.close()
