...
class Trajectory:
    def __init__(self, trajectory_length:int, burn_in_length:int):
    ...
    def verify_trajectory(self):
        # trajectory lenght must be at least 2. If it's lesss then take one more record from burn in
        if self.writing_burn_in or self.write_idx > 1:
            return
...
class R2D2_AgentBuffer(object):
    ...
    def store(self, actor_hidden_state:tf.Tensor, state:tf.Tensor, action:tf.Tensor, reward:tf.Tensor, is_terminal:tf.Tensor):
        ...
        if self.memory_idx > 0 and \
            ...
            not is_terminal > 0:
            self.trajectories.append(Trajectory(self._trajectory_size, self._burn_in_len))
