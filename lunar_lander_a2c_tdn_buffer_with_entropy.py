...
from rl_utils.SARST_NStepReturn_RandomAccess_MemoryBuffer_NoOverwrite import SARST_NStepReturn_RandomAccess_MemoryBuffer_NoOverwrite
...
exp_buffer = SARST_NStepReturn_RandomAccess_MemoryBuffer_NoOverwrite(1001, N_return, gamma, env.observation_space.shape, env.action_space.shape, np.int32)
...
def train_critic(states, next_states, rewards, gamma_powers, dones):
    with tf.GradientTape() as tape:
        ...
        next_state_values = tf.squeeze(critic(next_states, training=False))
        n_step_return = rewards + (1 - dones) * tf.math.pow(gamma, tf.cast(gamma_powers + 1, dtype=tf.float32)) * next_state_values
        
        advantage = n_step_return - current_state_value # TD(N) error
        loss = tf.reduce_mean(0.5 * tf.math.pow(advantage, 2)) #mse
...
for i in range(num_episodes):
    ...
    while not (done or trunc):
        actions_logits = actor(np.expand_dims(observation, axis = 0), training=False)[0]
        ...
        next_observation, reward, done, trunc, _ = env.step(chosen_action)
        episodic_reward += reward
        
        exp_buffer.store(observation, 
                         chosen_action, 
                         next_observation, 
                         reward, 
                         float(done or trunc))
