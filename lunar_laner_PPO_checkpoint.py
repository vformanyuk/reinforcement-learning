...
def train_critic(states, rewards, trajectory_len):
    ...
    with tf.GradientTape() as tape:
        ...
        for j in tf.range(end_idx, start_idx, delta = -1):
            ...
            current_gae = gae.assign_add(tf.math.pow(lambda_gamma_constant, gae_power) * tf.squeeze(delta - V[j]))
            #current_gae = gae.assign(tf.squeeze(delta - V[j]) + lambda_gamma_constant * gae.value())
